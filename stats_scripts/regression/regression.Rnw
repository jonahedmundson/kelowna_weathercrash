...
#plotting all
plot(density(test$crashes), main='', xlab='Num. of Crashes per Day', ylim=c(0, 0.016), lwd=4)
lines(density(glm.pred), col=2, lwd=2)
lines(density(nonp.pred), col=3, lwd=2)
lines(density(rf.pred), col=4, lwd=2)
lines(density(boost.pred), col=6, lwd=2)
legend("topright", 
       legend = c("Testing Data", "GLM", "Non-parametric", "Random Forest", "Boosting"),
       col = c(1:4, 6),
       lty = 1, lwd=2
)
@
...
lines(panel.smooth(regdata$visibility,regdata$crashes,lwd=2,span=0.8))
@
...
<<fig=TRUE>>=
plot(crashes~precip, data=regdata)
lines(panel.smooth(regdata$precip,regdata$crashes,lwd=2,span=0.8))
@
...
<<fig=TRUE>>=
cat('Reprint of plot from Variable Investigation:')
#reordering factor
...
#anova
cat('Crash-Casualty Ratio ANOVA:')
